...
class UserNotificationSender
{
    ...
    public function notificationToAll($request)
    {
        ...
        if (!$handleSelectedUser) {
            return $this->redirectWithNotify('error', "Ensure that the customer field is populated when sending an email to the designated user group");
        }
        ...
        if ($totalUserCount <= 0) {
            return $this->redirectWithNotify('error', "Notification recipients were not found among the selected customer base.");
        }
