...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $widget['totalOrders']        = Order::paid()->where('user_id', $user->id)->count();
        ...
        $widget['totalOrderAmount']   = Order::paid()->where('user_id', $user->id)->sum('subtotal');
        $widget['totalReturnedAmount']   = Order::paid()->where('status', Status::ORDER_PAYMENT_RETURNED)->where('user_id', $user->id)->sum('subtotal');
    ...
    public function sendNotificationAll(Request $request)
    {
        $request->validate([
            ...
            'number_of_top_order_customer' => 'required_if:being_sent_to,topOrderUsers|integer|gte:0',
            'number_of_days'               => 'required_if:being_sent_to,notLoginUsers|integer|gte:0',
            'image'                        => ["nullable", 'image', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
        ], [
            'number_of_days.required_if'               => "Number of days field is required",
            'number_of_top_order_customer.required_if' => "Number of top deposited user field is required",
        ]);
