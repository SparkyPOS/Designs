...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // vendor Info
        $widget['total_vendors']             = Vendor::count();
        $widget['verified_vendors']          = Vendor::active()->count();
        $widget['email_unverified_vendors']  = Vendor::emailUnverified()->count();
        $widget['mobile_unverified_vendors'] = Vendor::mobileUnverified()->count();
        ...
        // vendor Browsing, Country, Operating Log
        $venodrLoginData = VendorLogin::where('created_at', '>=', Carbon::now()->subDays(30))->get(['browser', 'os', 'country']);

        $chart['vendor_browser_counter'] = $venodrLoginData->groupBy('browser')->map(function ($item, $key) {
            return collect($item)->count();
        });
        $chart['vendor_os_counter'] = $venodrLoginData->groupBy('os')->map(function ($item, $key) {
            return collect($item)->count();
        });
        $chart['vendor_country_counter'] = $venodrLoginData->groupBy('country')->map(function ($item, $key) {
            return collect($item)->count();
        })->sort()->reverse()->take(5);


        $order['total_order_amount']        = Order::paid()->sum('subtotal');
        $order['total_order_pending']       = Order::paid()->pending()->count();
        $order['total_order_delivered']      = Order::paid()->delivered()->count();
        $order['total_order_charge']        = Order::paid()->sum('commission_amount');
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart','order','withdrawals'));
    }
