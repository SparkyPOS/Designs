@php
    $selectedData = json_decode($productPrintArea->selected_area ?? '');
@endphp
<div class="col-md-12">
    <input type="hidden" name="id[]" value="{{ $productPrintArea->id ?? null }}">
    <div class="card custom--card printarea--card">
        <div class="card-header variant-item d-flex align-items-center justify-content-between flex-wrap gap-2">
            ...
            <div class="d-flex align-items-center gap-3 variant-item-bottom mt-3 mb-2">
                @if (!($loop->first ?? true))
                <button type="button" class=" remove-btn removePrintArea"><i class="la la-times"></i>
                </button>
                @endif
                <a class="toggle-btn expandPrintAreaBtn" data-bs-toggle="collapse"
                    href="#printSideArea{{ $loop->index ?? 0 }}" role="button" aria-expanded="true"
                    aria-controls="printSideArea{{ $loop->index ?? 0 }}">
                    <i class="las la-angle-down"></i>
                </a>
            </div>
        </div>

        <div class="card-body printSideAreaItem collapse show" id="printSideArea{{ $loop->index ?? 0 }}">
            <div class="row gy-4">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form--label">@lang('Name')</label>
                        <input type="text" class="form--control" name="name[]"
                            value="{{ $productPrintArea->name ?? null }}" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form--label" for="regular_price">@lang('Print Area Width')</label>
                        <div class="input-group input--group">
                            <input type="number" class="form--control form-control" step="any"
                                name="print_area_width[]" value="{{ $selectedData->width ?? null }}" required>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form--label" for="regular_price">@lang('Print Area Height')</label>
                        <div class="input-group input--group">
                            <input type="number" class="form--control form-control" step="any"
                                name="print_area_height[]" value="{{ $selectedData->height ?? null }}" required>
                            <span class="input-group-text">@lang('Inches')</span>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="file" class="form--control"
                            data-src="{{ $productPrintArea->image ?? false ? getImage(getFilePath('printArea') . '/' . $productPrintArea->image) : null }}"
                            name="print_area_image[]" @required(($productPrintArea->image ?? false) == false)>
                        <small class="text--base-two">
                            <i class="la la-info-circle"></i>
                            @lang('Supported Files: .png, .jpg, .jpeg, .svg.')
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <input type="hidden" class="form--control" name="selected_area[]"
                            value="{{ $productPrintArea->selected_area ?? null }}" required>
