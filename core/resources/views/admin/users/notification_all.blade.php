            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ isset($sessionData['total_user']) ? $sessionData['total_user'] : '' }}" bg="primary" :viewMoreIcon=false />
                        </div>
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-envelope-circle-check" :title="$viaText . ' has been sent'" value="{{ isset($sessionData['total_sent']) ? $sessionData['total_sent'] : '' }}" bg="success" :viewMoreIcon=false />
                        </div>
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false value="{{ (isset($sessionData['total_user']) ? $sessionData['total_user'] : 0) - (isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0) }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-envelope" :title="$viaText . ' per batch'" value="{{ isset($sessionData['batch']) ? $sessionData['batch'] : '' }}" bg="primary" :viewMoreIcon=false />
                        </div>
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body p-5 text-center">
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        ...
                                        <p class="text--primary">
                                            {{ isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0 }} @lang('out of') {{ (isset($sessionData['total_user']) ? $sessionData['total_user'] : 0) . ' ' . $viaName }} @lang('were successfully transmitted').
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToUser as $key => $toUser)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', isset($sessionData['being_sent_to']) ? $sessionData['being_sent_to'] : '') == $key)>{{ __($toUser) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none userCountText"> <i class="las la-info-circle"></i> <strong class="userCount">0</strong> @lang('active customers found to send the notification')</small>
                                </div>
                            ...
                            <div class="form-group col-md-12 subject-wrapper">
                                ...
                                <input type="text" class="form-control" placeholder="@lang('Subject / Title')" name="subject" value="{{ old('subject', isset($sessionData['subject']) ? $sessionData['subject'] : '') }}" required>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', isset($sessionData['start']) ? $sessionData['start'] : '') }}" type="number" placeholder="@lang('Start form customer id. e.g. 1')" required>
                                        </div>
                                    </div>
                                    <div class="col-md-4 per-batch-col">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', isset($sessionData['batch']) ? $sessionData['batch'] : '') }}" type="number" placeholder="@lang('How many customer')" required>
                                    ...
                                    <div class="col-md-4 cooling-period-col">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="cooling_time" value="{{ old('cooling_time', isset($sessionData['batch']) ? $sessionData['batch'] : '') }}" type="number" placeholder="@lang('Waiting time')" required>
                                                <span class="input-group-text">
                                                    @lang('Seconds')
                                                </span>
                                            </div>
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'SELECTEDUSERS') {
                    ...
                    <div class="form-group" id="user_list_wrapper">
                        <label class="required">@lang('Select Customer')</label>
                ...
                if (methodName == 'TOPORDERUSERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Order Customer')</label>
                        <input class="form-control" type="number" name="number_of_top_order_customer" >
                    </div>
                    `);
                    return;
                }
