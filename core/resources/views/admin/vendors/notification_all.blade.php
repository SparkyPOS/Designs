...
@section('panel')
    @php
        ...
        $viaName     = isset($sessionData['via']) ? $sessionData['via'] : 'email';
        $viaText     = isset($sessionData['via']) && $sessionData['via'] == 'push' ? 'Push notification ' : ucfirst($viaName);
    @endphp
            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ isset($sessionData['total_vendor']) ? $sessionData['total_vendor'] : '' }}"
                        ...
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-envelope-circle-check" :title="$viaText . ' has been sent'"
                                value="{{ isset($sessionData['total_sent']) ? $sessionData['total_sent'] :'' }}" bg="success" :viewMoreIcon=false />
                        </div>
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false
                                value="{{ (isset($sessionData['total_vendor']) ? $sessionData['total_vendor'] : 0) - (isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0) }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-envelope" :title="$viaText . ' per batch'" value="{{ isset($sessionData['batch']) ? $sessionData['batch'] : '' }}"
                                bg="primary" :viewMoreIcon=false />
                        </div>
                        <div class="col-12">
                            <div class="card">
                                    ...
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        <div class="countdown">
                                            <div class="coaling-time">
                                                <span class="coaling-time-count">{{ isset($sessionData['cooling_time']) ? $sessionData['cooling_time'] : '' }}</span>
                                            </div>
                                            <div class="svg-count">
                                        ...
                                        <p class="text--primary">
                                            {{ (isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0) }} @lang('out of') {{ (isset($sessionData['total_vendor']) ? $sessionData['total_vendor'] : 0) . ' ' . $viaName }} @lang('were successfully transmitted').
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToVendor as $key => $toVendor)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', (isset($sessionData['being_sent_to']) ? $sessionData['being_sent_to'] : '')) == $key)>{{ __($toVendor) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none vendorCountText"> <i class="las la-info-circle"></i> <strong
                                            class="vendorCount">0</strong> @lang('active vendors found to send the notification')</small>
                                </div>
                            ...
                            <div class="form-group col-md-12 subject-wrapper">
                                ...
                                <input type="text" class="form-control" placeholder="@lang('Subject / Title')" name="subject"
                                    value="{{ old('subject', isset($sessionData['subject']) ? $sessionData['subject'] : '') }}">
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control nicEdit" id="nicEdit" name="message" rows="10">{{ old('message', isset($sessionData['message']) ? $sessionData['message'] : '') }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', isset($sessionData['start']) ? $sessionData['start'] : '') }}"
                                                type="number" placeholder="@lang('Start form vendor id. e.g. 1')" required>
                                        </div>
                                    </div>
                                    <div class="col-md-4 per-batch-col">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', isset($sessionData['batch']) ? $sessionData['batch'] : '') }}"
                                                    type="number" placeholder="@lang('How many vendor')" required>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="cooling_time"
                                                    value="{{ old('cooling_time', isset($sessionData['batch']) ? $sessionData['batch'] : '') }}" type="number"
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                getVendorCount(methodName);
                methodName = methodName.toUpperCase();

                if (methodName == 'SELECTEDVENDORS') {
                    $('.input-append').html(`
                    <div class="form-group" id="vendor_list_wrapper">
                        <label class="required">@lang('Select vendor')</label>
                        <select name="vendor[]"  class="form-control" id="vendor_list" required multiple >
                ...
                if (methodName == 'TOPDEPOSITEDVENDORS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Order vendor')</label>
                        <input class="form-control" type="number" name="number_of_top_order_vendor" >
                    </div>
                    `);
                    return;
                }

                if (methodName == 'NOTLOGINVENDORS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Days')</label>
                        <div class="input-group">
                            <input class="form-control" value="{{ old('number_of_days', isset($sessionData['number_of_days']) ? $sessionData['number_of_days'] : '') }}" type="number" name="number_of_days" >
                ...
                $('.row #vendor_list').select2({
                    ajax: {
                        ...
                        processResults: function(response, params) {
                            ...
                            let data = response.vendors.data;
                            return {
                    ...
                    dropdownParent: $('.input-append #vendor_list_wrapper')
                });

            }


            function getVendorCount(methodName) {
                var methodNameUpper = methodName.toUpperCase();
                if (methodNameUpper == 'SELECTEDVENDORS' || methodNameUpper == 'ALLVENDORS' || methodNameUpper == 'TOPDEPOSITEDVENDORS' ||
                    methodNameUpper == 'NOTLOGINVENDORS') {
                    $('.vendorCount').text(0);
                    $('.vendorCountText').addClass('d-none');
                    return;
                }
                var route = "{{ route('admin.vendors.segment.count', ':methodName') }}"
                route = route.replace(':methodName', methodName)
                $.get(route, function(response) {
                    $('.vendorCount').text(response);
                    $('.vendorCountText').removeClass('d-none');
                });
            }
        ...
        @if (!empty(isset($sessionData)) && @request()->email_sent && @request()->email_sent = 'yes')
